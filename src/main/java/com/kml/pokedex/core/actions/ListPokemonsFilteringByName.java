...
public class ListPokemonsFilteringByName {
  ...
  public List<Pokemon> invoke(String queryFilter) {
    if(queryFilter.trim().length()<=3) throw new InvalidNameFilter("Filter must have more that 3 letter");
    return pokemonsRepository.findAllWithNameLike(queryFilter);
  }
